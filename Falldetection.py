...
def process_webcam():
    ...
    vid_cap = cv2.VideoCapture(1)  # Using default webcam (index 0)
    if not vid_cap.isOpened():
        print('Error while trying to open webcam.')
        return

    # Prepare video writer (output video stream setup)
    vid_out = None  # No output video for webcam
    ...
    while True:
        ...
        if not success:
            print('Error while trying to read frame from webcam.')
            break
        ...
        if is_fall:
            if fall_start_time is None:
                fall_start_time = time.time()  # Record start time of fall
            else:
                elapsed_time = time.time() - fall_start_time
                if elapsed_time >= 5.0:  # If fallen for more than 5 seconds
                    falling_alarm(_image, bbox, elapsed_time)
        else:
            fall_start_time = None  # Reset fall start time if not fallen
        
        cv2.imshow('Webcam Feed', _image)
...
def falling_alarm(image, bbox, elapsed_time):
    ...
    cv2.putText(image, 'Fallen for {:.2f}s'.format(elapsed_time), (11, 130), 0, 1, [0, 0, 255], thickness=3,
                lineType=cv2.LINE_AA)

def get_pose_model():
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    weights = torch.load('C:/Users/matia/Human-Fall-Detection-master/yolov7-w6-pose.pt',map_location=device)
    model = weights['model']
    _ = model.float().eval()
...
def process_video(video_path):
    ...
    fall_start_time = 0  # 쓰러진 시간 기록 변수
    ...
    for image in tqdm(_frames):
        ...
        if is_fall:
            if fall_start_time is None:
                fall_start_time = time.time()  # 쓰러진 시작 시간 기록
            else:
                elapsed_time = time.time() - fall_start_time
                if elapsed_time >= 5.0:  # 5초 이상 쓰러진 경우
                    falling_alarm(_image, bbox, elapsed_time)

        else:
            fall_start_time = None  # 쓰러진 상태가 아닌 경우 초기화
